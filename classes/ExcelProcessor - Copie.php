...
class ExcelProcessor
{
    ...
    private function updateProgress($percentage, $message = '')
    {
        ...
        // Petit délai pour rendre la progression visible (200ms)
        usleep(200000);
    }
    ...
    public function mergeExcelFiles()
    {
        try {
            ...
            // Scanner les fichiers du dossier uploads
            ...
            $tempFilePath = TEMP_DIR . '/' . $outputFileName;
    ...
    private function createSpecializedSheets($spreadsheet, $fusionSheet, $headerRows)
    {
        ...
        // Création de la feuille récapitulative
        $this->updateProgress(88, 'Création de la feuille récapitulative...');
            $this->createRecapSheet($spreadsheet, $payerData, $npData);
            // Mettre la feuille RECAP FIN en première position
            $recapIndex = $spreadsheet->getIndex($spreadsheet->getSheetByName('RECAP FIN'));
            if ($recapIndex !== 0) {
                $recapSheet = $spreadsheet->getSheet($recapIndex);
                // Supprimer la feuille RECAP FIN de sa position actuelle
                $spreadsheet->removeSheetByIndex($recapIndex);
                // Réinsérer la feuille RECAP FIN en première position
                $spreadsheet->addSheet($recapSheet, 0);
    ...
    private function createPayerSheet($spreadsheet, $payerData, $headerRows)
    {
        ...
        $this->configureSheet($payerSheet, 'LISTE DES BENEFICIAIRES PAYES EN TOTALITE', $payerData);
    }
    ...
    private function createRecapSheet($spreadsheet, $payerData, $npData)
    {
        ...
        if (file_exists($logoPath2)) {
            ...
            $drawing2->setHeight(125); // 3.31 cm
        ...
        // Ligne 7: Transfert monétaire FSP
        $recapSheet->mergeCells('A7:I7')->setCellValue('A7', 'TRANSFERT MONETAIRE FSP');
        ...
        $recapSheet->setCellValue('A8', 'REGION :');
        $recapSheet->setCellValue('D8', 'DISTRICT :');
        ...
        $recapSheet->mergeCells('A9:B9')->setCellValue('A9', 'TERROIR');
        ...
        // Fusion de toutes les cellules A en une grande cellule vide (jaune)
